...
async function loadNominalTable() {
  ...
  // Tambahkan tombol List Jual
  const listJualButton = document.createElement('button');
  listJualButton.textContent = 'List Jual';
  listJualButton.onclick = () => window.location.href = 'list jual.html';
  listJualButton.style.marginTop = '10px';
  listJualButton.style.marginBottom = '10px';
  listJualButton.style.padding = '8px 12px';
  listJualButton.style.backgroundColor = '#006600';
  listJualButton.style.color = 'white';
  listJualButton.style.border = 'none';
  listJualButton.style.borderRadius = '6px';
  listJualButton.style.cursor = 'pointer';
  
  totalDiv.appendChild(listJualButton);
  ...
  Object.values(latestByWebsite).forEach(row => {
    ...
    tr.innerHTML = `
      <td>${new Date(row.created_at).toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}</td>
